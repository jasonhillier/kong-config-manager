...
const SON_OBJECTS = ['routes', 'plugins'];

module.exports = function apply(dir, host, ssl, nodelete) {
  debug(`Apply with [ssl checks=${ssl}]`);
  function core(obj) {
    ...
    return rp({
      ...
      .tap(remoteObjs => {
        ...
        // make 3 sets
        var itemsToDelete = _.without.apply(
          null,
          _.concat([remoteIds], itemIds)
        );
        if (nodelete)
        {
          debug('nodelete flag set, clearing delete list!');
          itemsToDelete = [];
        }
